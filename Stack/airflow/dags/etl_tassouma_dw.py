...
# ----------------------------------------------------------------------------
# 1. CONFIGURATIONS DES CHAINES DE CONNEXION (ON-PREMISE / OPEN SOURCE)
# ----------------------------------------------------------------------------
# Source: Application Tassouma (Django / MySQL)
# Source: Application Tassouma (L'alias dans le compose est mysql_oltp, mot de passe 'password')
URL_SOURCE_OLTP = "mysql+pymysql://root:password@mysql_oltp:3306/tassouma_oltp"

# Destination: Data Warehouse (L'alias dans le compose est postgres_dw, mot de passe 'postgres')
URL_CIBLE_DW = "postgresql+psycopg2://postgres:postgres@aes_postgres_dw:5432/aes_bi_dw"
...
def execution_pipeline_etl():
    ...
    try:
        ...
        # Implémentation des règles de gestion financières non stockées en base (Dictionnaire des données)
        ...
        # Gestion des commandes reçues ou en attente
        df_log_flat['id_temps_reception'] = pd.to_datetime(df_log_flat['updated_at_e']).dt.strftime('%Y%m%d')
        df_log_flat['id_temps_reception'] = df_log_flat['id_temps_reception'].fillna(-1).astype(float).astype(int)
        df_log_flat['id_temps_reception'] = df_log_flat['id_temps_reception'].replace(-1, None)

        # Calcul du délai d'approvisionnement en jours
        delta_days = (pd.to_datetime(df_log_flat['updated_at_e']) - pd.to_datetime(df_log_flat['created_at_e'])).dt.days
        df_log_flat['delai_approvisionnement_jours'] = delta_days.fillna(-1).astype(int).replace(-1, None)
